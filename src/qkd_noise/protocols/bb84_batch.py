import numpy as np

from qiskit import QuantumCircuit
from qiskit_aer import AerSimulator
from qiskit.quantum_info import Kraus

from qkd_noise.channels import (
    depolarizing_kraus,
    dephasing_kraus,
    amplitude_damping_kraus,
)


def build_bb84_circuit(
    alice_bit: int,
    alice_basis: int,
    bob_basis: int,
    p: float,
    scenario: str,
) -> QuantumCircuit:
    """
    Build one BB84 transmission circuit.

    Basis:
        0 = Z basis
        1 = X basis

    Scenario:
        single = depolarizing
        dual   = depolarizing -> dephasing
        triple = depolarizing -> dephasing -> amplitude damping
    """

    if scenario not in {"single", "dual", "triple"}:
        raise ValueError(
            "scenario must be 'single', 'dual', or 'triple'"
        )

    qc = QuantumCircuit(1, 1)

    # ---------------------------------------------------------
    # Alice: prepare BB84 state
    # ---------------------------------------------------------

    # Bit = 1 changes |0> -> |1>
    if alice_bit == 1:
        qc.x(0)

    # X basis:
    # |0> -> |+>
    # |1> -> |->
    if alice_basis == 1:
        qc.h(0)

    # ---------------------------------------------------------
    # Exact paper noise channels
    # ---------------------------------------------------------

    # Depolarizing
    qc.append(
        Kraus(depolarizing_kraus(p)),
        [0],
    )

    # Dephasing
    if scenario in {"dual", "triple"}:
        qc.append(
            Kraus(dephasing_kraus(p)),
            [0],
        )

    # Amplitude damping
    if scenario == "triple":
        qc.append(
            Kraus(amplitude_damping_kraus(p)),
            [0],
        )

    # ---------------------------------------------------------
    # Bob: measurement basis
    # ---------------------------------------------------------

    # To measure in X basis, rotate X -> Z.
    if bob_basis == 1:
        qc.h(0)

    qc.measure(0, 0)

    return qc


def simulate_bb84_batch(
    n: int = 1000,
    p: float = 0.05,
    scenario: str = "single",
    seed: int | None = 12345,
):
    """
    Run n independent BB84 circuits in ONE AerSimulator job.

    Returns:
        Alice/Bob data,
        sifted data,
        QBER,
        number of sifted bits.
    """

    rng = np.random.default_rng(seed)

    # ---------------------------------------------------------
    # Alice and Bob random choices
    # ---------------------------------------------------------

    alice_bits = rng.integers(0, 2, size=n)
    alice_bases = rng.integers(0, 2, size=n)
    bob_bases = rng.integers(0, 2, size=n)

    circuits = []

    for i in range(n):

        qc = build_bb84_circuit(
            alice_bit=int(alice_bits[i]),
            alice_basis=int(alice_bases[i]),
            bob_basis=int(bob_bases[i]),
            p=p,
            scenario=scenario,
        )

        circuits.append(qc)

    # ---------------------------------------------------------
    # ONE Aer job for all circuits
    # ---------------------------------------------------------

    backend = AerSimulator()

    result = backend.run(
        circuits,
        shots=1,
        seed_simulator=seed,
    ).result()

    # ---------------------------------------------------------
    # Extract Bob's measurement results
    # ---------------------------------------------------------

    bob_bits = np.empty(n, dtype=int)

    for i in range(n):

        counts = result.get_counts(i)

        outcome = next(iter(counts))

        bob_bits[i] = int(outcome)

    # ---------------------------------------------------------
    # Sifting
    # ---------------------------------------------------------

    sift_mask = alice_bases == bob_bases

    alice_sifted = alice_bits[sift_mask]
    bob_sifted = bob_bits[sift_mask]

    # ---------------------------------------------------------
    # QBER
    # ---------------------------------------------------------

    if len(alice_sifted) == 0:
        qber = 0.0
    else:
        qber = float(
            np.mean(alice_sifted != bob_sifted)
        )

    return {
        "alice_bits": alice_bits,
        "alice_bases": alice_bases,
        "bob_bases": bob_bases,
        "bob_bits": bob_bits,
        "sift_mask": sift_mask,
        "alice_sifted": alice_sifted,
        "bob_sifted": bob_sifted,
        "qber": qber,
        "sifted_length": len(alice_sifted),
        "total_bits": n,
    }
